...
# Function to get Facebook Pages and their Access Tokens (Handles Missing Access Tokens)
...
async def get_page_insights(session, page_id, page_token, start_date, end_date):
    ...
    async with session.get(url) as response:
        try:
            ...
            logging.error(f"Error parsing JSON for Page ID {page_id}: {e}")
            response_json = {}

        logging.info(f"API Response for {page_id}: {response_json}")  # ✅ Debug response
...
async def main_pages():
    async with aiohttp.ClientSession() as session:
        ...
        end_date = "2025-01-30"  # Replace with your desired end date
...
async def get_posts_for_page(session, page_id, page_token, start_date, end_date):
    url = f"https://graph.facebook.com/v22.0/{page_id}/posts?fields=id,created_time&since={start_date}&until={end_date}&access_token={page_token}"
    posts=[]
    ...
    while url:
        async with session.get(url) as response:
            try:
                ...
                logging.error(f"Error parsing JSON for posts on Page {page_id}: {e}")
                break

            # Debugging: Print response
            logging.info(f"API Response for posts on Page {page_id}: {response_json}")
...
async def get_post_insights(session, post_id, page_id,date_posted, start_date, end_date, page_token):
    url = f"https://graph.facebook.com/v22.0/{post_id}/insights?metric={','.join(post_metrics)}&access_token={page_token}"

    async with session.get(url) as response:
        try:
            response_json = await response.json()
        except Exception as e:
            logging.error(f"Error parsing JSON for Post {post_id} on Page {page_id}: {e}")
            return {}

        logging.info(f"API Response for Post {post_id} on Page {page_id}: {response_json}")
